...
const CUDA_INIT_RAN_PATTERN = /cuInit\(0\)=/;
...
export function createDirectSandboxGpuVerifier(
  ...
  return function verifyDirectSandboxGpu(
    ...
    for (const proof of buildProofCommands(sandboxName)) {
      ...
      const cudaInitRan = CUDA_INIT_RAN_PATTERN.test(rawOutput);
      if (result.status === 0) {
        console.log(`  ✓ GPU proof passed: ${proof.label}`);
        // Require the cuInit(0)= marker on success too, symmetric with the
        // failure path: a zero exit without driver initialization (e.g. a shell
        // wrapper that swallowed the real exit code) must not read as verified.
        if (proof.id === CUDA_USABILITY_PROOF_ID && cudaInitRan) cudaVerified = true;
        continue;
      }
